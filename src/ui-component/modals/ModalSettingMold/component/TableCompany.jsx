...
import { Table, TableBody, styled, TableCell, TableContainer, TableHead, TableRow, Paper, tableCellClasses, IconButton, Box, Grid, Button, TextField, Stack, FormControl, OutlinedInput, InputAdornment, TablePagination, Tooltip } from "@mui/material"
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={4}>
                        <Controller
                            ...
                            control={control}
                            rules={{ required: ' ' }}
                            render={({ field }) => <TextField fullWidth size="small" {...field} label="Code" variant="outlined" placeholder="Code..." error={!!errors.codeCompany} />}
                        />
                    </Grid>
                    <Grid item xs={5}>
                        <Controller
                            ...
                            control={control}
                            rules={{ required: ' ' }}
                            render={({ field }) => <TextField fullWidth size="small" {...field} label="Description" placeholder="Description..." variant="outlined" error={!!errors.nameCompany} />}
                        />
                    </Grid>
            ...
            <Stack direction={'row'} justifyContent={'flex-end'}>
                <FormControl size="small" variant="outlined">
                    <OutlinedInput
                        ...
                        onBlur={(e) => { setSearchCompany(e.target.value) }}
                        placeholder="Search..."
                        type={'text'}
                        endAdornment={
                            <InputAdornment position="end">
                                <IconButton color="primary" aria-label="search" onClick={() => { onClickSearch('COMPANY') }} edge="end">
                                    <IconSearch />
                                </IconButton>
                            </InputAdornment>
                        }
                ...
                <Table style={{ tableLayout: 'fixed' }} stickyHeader aria-label="customized table">
                    <TableHead>
                        <TableRow>
                            ...
                            <StyledTableCell align="center" width={130}>Code</StyledTableCell>
                                    ...
                                    {typeModal === 'PICK' ?
                                        (
                                            <Tooltip title="Click to choose item">
                                                <IconButton onClick={()=>{onChooseItem(company,'MODEL')}} size="small" color="primary" >
                                                    <IconCheckbox />
                                                </IconButton>
                                            </Tooltip>
                                        ) :
                                            ...
                                            isEditing === company?.companyID ?
                                                (
                                                    <Tooltip title="Cancel edit">
                                                        ...
                                                        <IconButton color="error" onClick={() => { onClickCancel(company) }} size="small" >
                                                            <IconCircleX />
                                                        </IconButton>
                                                    </Tooltip>
                                                )
                                                :
                                                (<IconButton onClick={() => { onClickEdit(company) }} size="small" >
                                                    <IconEdit />
                                                </IconButton>)
                                        )}
                                </StyledTableCell>
