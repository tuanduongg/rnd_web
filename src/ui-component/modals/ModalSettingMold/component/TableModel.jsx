...
import { Table, TableBody, styled, TableCell, TableContainer, TableHead, TableRow, Paper, tableCellClasses, IconButton, Stack, Grid, Box, TextField, Button, FormControl, InputLabel, Select, MenuItem, OutlinedInput, InputAdornment, TablePagination, Tooltip } from "@mui/material"
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={3}>
                        <Controller
                            ...
                            control={control}
                            rules={{ required: ' ' }}
                            render={({ field }) => <TextField size="small" {...field} label="Project name" variant="outlined" error={!!errors.project} />}
                        />
                    </Grid>
                    <Grid item xs={3}>
                        <Controller
                    ...
                    <Grid item xs={12} textAlign={'right'}>
                        <Controller
                            ...
                            control={control}
                            rules={{ required: ' ' }}
                            render={({ field }) => <TextField fullWidth size="small" {...field} label="Description" variant="outlined" error={!!errors.description} />}
                        />
                    </Grid>
                    <Grid item xs={12} textAlign={'right'}>
            ...
            <Stack direction={'row'} justifyContent={'flex-end'}>
                <FormControl size="small" variant="outlined">
                    <OutlinedInput
                        ...
                        onBlur={(e) => { setSearchModel(e.target.value) }}
                        placeholder="Search..."
                        type={'text'}
                        endAdornment={
                            <InputAdornment position="end">
                                <IconButton color="primary" aria-label="search" onClick={() => { onClickSearch('MODEL') }} edge="end">
                                    <IconSearch />
                                </IconButton>
                            </InputAdornment>
                        }
                ...
                <Table style={{ tableLayout: 'fixed' }} stickyHeader aria-label="customized table">
                    <TableHead>
                        <TableRow>
                            ...
                            <StyledTableCell align="center" width={100}>Category</StyledTableCell>
                            <StyledTableCell align="center" sx={{ width: '120px' }}>Project Name</StyledTableCell>
                            <StyledTableCell align="center" width={100} >구분</StyledTableCell>
                            <StyledTableCell align="center" width={100} >Model</StyledTableCell>
                            <StyledTableCell align="center">Description</StyledTableCell>
                            <StyledTableCell align="right" width={50} >{' '}</StyledTableCell>
                    ...
                    >
                        {modelMolds?.length > 0 ? (modelMolds.map((item, index) => (
                                    ...
                                    {typeModal === 'PICK' ?
                                        (
                                            <Tooltip title="Click to choose item">
                                                <IconButton size="small" onClick={()=>{onChooseItem(item,'MODEL')}} color="primary" >
                                                    <IconCheckbox />
                                                </IconButton>
                                            </Tooltip>
                                        ) :
                                            ...
                                            isEditing === item?.modelID ?
                                                (
                                                    <Tooltip title="Cancel edit">
                                                        ...
                                                        <IconButton color="error" onClick={() => { onClickCancel(item) }} size="small" >
                                                            <IconCircleX />
                                                        </IconButton>
                                                    </Tooltip>
                                                )
                                                :
                                                (<IconButton onClick={() => { onClickEdit(item) }} size="small" >
                                                    <IconEdit />
                                                </IconButton>)
                                        )}
                                </StyledTableCell>
