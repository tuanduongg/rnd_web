...
const downloadSlice = createSlice({
  ...
  reducers: {
    ...
    removeDownload: (state, action) => {
      const files = state.files.filter((item) => item?.id !== action.payload?.id);
      state.files = files;
    },
...
export const { addDownload, updateDownloadProgress, toggleMenu, downloadSuccessful,removeDownload } = downloadSlice.actions;
